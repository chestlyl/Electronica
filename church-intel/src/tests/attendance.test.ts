...
async function main() {
  ...
  check('inferred: 3-campus multi-site floors at ~1,800 (not 850)', () => assert.ok((c3.attendance_estimate.value ?? 0) >= 1800, `got ${c3.attendance_estimate.value}`));
  ...
  check('inferred: 9-campus floors ~5,400 (Grace-calibrated ≈ reported 5,372)', () => assert.ok((c9.attendance_estimate.value ?? 0) >= 5400, `got ${c9.attendance_estimate.value}`));
